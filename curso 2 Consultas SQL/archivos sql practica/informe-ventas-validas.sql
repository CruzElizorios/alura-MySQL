...
/*CANTIDAD DE VENTAS POR MES PARA CLIENTE*/
...
SUM(IFa.CANTIDAD) AS CANTIDAD_VENDIDA, MAX(VOLUMEN_DE_COMPRA)/10 AS CANTIDAD_MAX FROM facturas F
INNER JOIN 
items_facturas IFa ON F.NUMERO = IFa.NUMERO
...
CASE 
	WHEN (A.CANTIDAD_VENDIDA - A.CANTIDAD_MAX) <= 0 THEN 'VENTA_VALIDA'
    ELSE 'VENTA inVALIDA'
END AS STATUS_VENTA
 FROM(
SELECT F.DNI, TC.NOMBRE,DATE_FORMAT(F.FECHA_VENTA, "%m - %Y") AS MES_AÑO,
